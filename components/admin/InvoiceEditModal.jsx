...
const InvoiceEditModal = ({ invoice, onClose, onSave }) => {
  ...
  const [formData, setFormData] = useState({
    type: invoice.type || 'customer',
    entityId: invoice.customerId || invoice.carrierId || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const invoiceData = {
        id: invoice.id,  // Mevcut fatura id'si
        type: formData.type,
      ...
      // Diğer tipin ID'sini temizle (müşteri ise taşıyıcı, taşıyıcı ise müşteri)
      if (formData.type === 'customer') {
        delete invoiceData.carrierId;
      } else {
      ...
      setError('Fatura güncellenirken bir hata oluştu');
      console.error(err);
    } finally {
      setLoading(false);
    }
      ...
      <div className="bg-white rounded-lg shadow-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-gray-800">Fatura Düzenle</h2>
            <button
              onClick={onClose}
